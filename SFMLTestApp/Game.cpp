...
void Game::Run()
{
    ...
    while (mWindow.isOpen())    // свЄрнутое окно считаетс€ true
    {
        ...
        currSteoOfTime = clock.restart();                    // «аписываем прошедшее врем€ на таймере и сбрасываем таймер на 0
        countTick++;
        ...
        while (timeSinceLastUpdate >= TimePerFrame) {        // ѕрошло достаточно времени дл€ обновлени€ кадра
            ...
            processEvents();     
            update(timeSinceLastUpdate + TimePerFrame, TimePerFrame, countTick);       // ќбрабатываем новую итерацию цикла жизни игры, физический тик игры
            countTick = 0;
        }
